...
![144](https://ws4.sinaimg.cn/large/006tKfTcly1g09s196pxpj30rs0efq41.jpg)
...
从这些特征中可以看出，图 8-1 的网络中使用了多个之前介绍的神经网络技术。现在，我们使用这个网络进行学习。先说一下结论，这个网络的识别精度为 99.38% {1[最终的识别精度有少许偏差，不过在这个网络中，识别精度大体上都会超过 99%。]}，可以说是非常优秀的性能了！

### 进一步提高识别精度

在一个标题为“What is the class of this image ?”的网站 [32] 上，以排行榜的形式刊登了目前为止通过论文等渠道发表的针对各种数据集的方法的识别精度（图 8-3）。

![146](https://ws1.sinaimg.cn/large/006tKfTcly1g09s5nlvoyj30rs0mstgq.jpg)

**图 8-3　针对 MNIST 数据集的各种方法的排行（引自文献 [32]：2016 年 6 月）**

显眼。实际上，排行榜上的前几名大都是基于 CNN 的方法。顺便说一下，截止到 2016 年 6 月，对 MNIST 数据集的最高识别精度是 99.79%（错误识别率为 0.21%），该方法也是以 CNN 为基础的 [33]。不过，它用的 CNN 并不是特别深层的网络（卷积层为 2 层、全连接层为 2 层的网络）。
...
![147](https://ws2.sinaimg.cn/large/006tKfTcly1g09s71ldgbj30rs09140d.jpg)

**图 8-4　Data Augmentation 的例子**

除了如图 8-4 所示的变形之外，Data Augmentation 还可以通过其他各种方法扩充图像，比如裁剪图像的“crop 处理”、将图像左右翻转的“flip 处理”{2[flip 处理只在不需要考虑图像对称性的情况下有效。]}等。对于一般的图像，施加亮度等外观上的变化、放大缩小等尺度上的变化也是有效的。不管怎样，通过 Data Augmentation 巧妙地增加训练图像，就可以提高深度学习的识别精度。虽然这个看上去只是一个简单的技巧，不过经常会有很好的效果。这里，我们不进行 Data Augmentation 的实现，不过这个技巧的实现比较简单，有兴趣的读者请自己试一下。

### 加深层的动机

关于加深层的重要性，现状是理论研究还不够透彻。尽管目前相关理论还比较贫乏，但是有几点可以从过往的研究和实验中得以解释（虽然有一些直观）。本节就加深层的重要性，给出一些增补性的数据和说明。

首先，从以 ILSVRC([ImageNet Large Scale Visual Recognition Competition](http://www.image-net.org/challenges/LSVRC/)) 为代表的大规模图像识别的比赛结果中可以看出加深层的重要性（详细内容请参考下一节）。这种比赛的结果显示，最近前几名的方法多是基于深度学习的，并且有逐渐加深网络的层的趋势。也就是说，可以看到层越深，识别性能也越高。
...
![148](https://ws1.sinaimg.cn/large/006tKfTcly1g09sa2n35cj30rs0d2mys.jpg)
...
![149](https://ws3.sinaimg.cn/large/006tKfTcly1g09sbkicufj30rs08jwg3.jpg)
...
![150](https://ws4.sinaimg.cn/large/006tKfTcly1g09sew4z9tj30rs09dan6.jpg)
...
![152](https://ws1.sinaimg.cn/large/006tKfTcly1g09sjes7wzj30rs0ci0ux.jpg)
...
![153](https://ws1.sinaimg.cn/large/006tKfTcly1g09skjmfk2j30rs065aci.jpg)
...
![154](https://ws4.sinaimg.cn/large/006tKfTcly1g09slvuonqj30rs0d3tam.jpg)
...
![155](https://ws2.sinaimg.cn/large/006tKfTcly1g09sohy3gkj30rs0fp0v1.jpg)
...
![156](https://ws1.sinaimg.cn/large/006tKfTcly1g09spjhmazj30rs03umzu.jpg)
...
![157](https://ws3.sinaimg.cn/large/006tKfTcly1g09srrzgnpj30rs09kwin.jpg)
...
![158](https://ws3.sinaimg.cn/large/006tKfTcly1g09ssz0cfpj30rs0iedho.jpg)
...
![159](https://ws3.sinaimg.cn/large/006tKfTcly1g09su1uq91j30rs0ftdh9.jpg)
...
```text
NVIDIA 的 Maxwell GPU 虽然支持半精度浮点数的存储（保存数据的功能），但是运算本身不是用 16 位进行的。下一代的 Pascal 框架，因为运算也是用 16 位进行的，所以只用半精度浮点数进行计算，就有望实现超过上一代 GPU 约 2 倍的高速化。
```

以往的深度学习的实现中并没有注意数值的精度，不过 Python 中一般使用 64 位的浮点数。NumPy 中提供了 16 位的半精度浮点数类型（不过，只有 16 位类型的存储，运算本身不用 16 位进行），即便使用 NumPy 的半精度浮点数，识别精度也不会下降。
...
![160](https://ws3.sinaimg.cn/large/006tKfTcly1g09sx1gedhj30rs0dhdyo.jpg)
...
![161](https://ws4.sinaimg.cn/large/006tKfTcly1g09sxu3ogyj30rs07w45a.jpg)
...
![162](https://ws2.sinaimg.cn/large/006tKfTcly1g09sz7u0wvj30rs0ai7bu.jpg)
...
![163](https://ws2.sinaimg.cn/large/006tKfTcly1g09t0t41j0j30rs0ejn2f.jpg)
...
![164](https://ws4.sinaimg.cn/large/006tKfTcly1g09t24ib3cj30rs0hd7rq.jpg)
...
![165](https://ws3.sinaimg.cn/large/006tKfTcly1g09t2qppkcj30rs0audpu.jpg)

**图 8-22　Neural Image Caption（NIC）的整体结构**
...
![166](https://ws4.sinaimg.cn/large/006tKfTcly1g09t48nvogj30rs0mt7wh.jpg)
...
![167](https://ws1.sinaimg.cn/large/006tKfTcly1g09t5jzlygj30rs0dxat1.jpg)
...
![168](https://ws1.sinaimg.cn/large/006tKfTcly1g09t6vq8a4j30rs0d7n6y.jpg)
...
![169](https://ws2.sinaimg.cn/large/006tKfTcly1g09t7mnjiej30rp0nqaa2.jpg)
...
![170](https://ws4.sinaimg.cn/large/006tKfTcly1g09t8zucn4j30rs0g1q84.jpg)
